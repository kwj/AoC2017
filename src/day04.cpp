...
bool
isValid(std::vector<std::string> const &vs) {
...
long
part1(std::vector<std::vector<std::string>> &vvs) {
    return static_cast<long>(std::count_if(vvs.begin(), vvs.end(), isValid));
}

long
part2(std::vector<std::vector<std::string>> &vvs) {
    ...
    return static_cast<long>(
        std::count_if(tmp_vvs.begin(), tmp_vvs.end(), isValid)
    );
}
